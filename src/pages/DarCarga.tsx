    ...
    >
      {step === 1 && (
        <div className="space-y-3">
          ...
          <OptionCard
            ...
            onClick={() => setTipoEnvio("encomienda")}
          ...
          <OptionCard
            ...
            onClick={() => setTipoEnvio("carga-general")}
      ...
      {step === 3 && tipoEnvio === "encomienda" && (
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input type="number" placeholder="Ej: 0.5" value={m3} onChange={(e) => setM3(e.target.value)} />
          </div>
        </div>
      )}
      ...
      {step === 4 && tipoEnvio === "carga-general" && (
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>{unidad === "pallets" ? "Cantidad de pallets" : "Cantidad y m³ aprox"}</Label>
            <Input type="number" placeholder="Ej: 2" value={cantidad} onChange={(e) => setCantidad(e.target.value)} />
            {unidad === "bultos" && (
              <>
                <Label>m³ aproximados</Label>
                <Input type="number" placeholder="Ej: 1.5" value={m3} onChange={(e) => setM3(e.target.value)} />
              </>
            )}
          </div>
        </div>
      ...
      {((tipoEnvio === "encomienda" && step === 4) || (tipoEnvio === "carga-general" && step === 5)) && (
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input placeholder="Ej: San Marcos Sierras, calle..." value={origen} onChange={(e) => setOrigen(e.target.value)} />
          </div>
        </div>
      )}
      ...
      {((tipoEnvio === "encomienda" && step === 5) || (tipoEnvio === "carga-general" && step === 6)) && (
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Dirección o referencia</Label>
            <Input placeholder="Ej: Córdoba Capital, barrio..." value={destino} onChange={(e) => setDestino(e.target.value)} />
          </div>
        </div>
      )}

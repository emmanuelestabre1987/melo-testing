  ...
  const renderPubSummary = (pub: { operation_type: string; data: Json }, small?: boolean) => {
    ...
    return (
      <div className="flex items-center gap-2.5">
        <div className={`flex ${small ? "h-8 w-8" : "h-9 w-9"} shrink-0 items-center justify-center rounded-xl ${config.gradient}`}>
          ...
          <div className="mx-auto max-w-lg space-y-5">
            {loading ? (
              ...
              <div className="py-16 text-center">
                ...
                <Button className="mt-5 rounded-full" variant="outline" onClick={() => navigate("/tablero")}>
                  Ir al tablero
                </Button>
              </div>
            ) : (
              <>
                {incoming.length > 0 && (
                  <div className="space-y-3">
                    <h3 className="text-sm font-bold text-foreground">Solicitudes recibidas</h3>
                    {incoming.map((m) => {
                      const st = statusLabels[m.status] || statusLabels.pending;
                      return (
                        <Card key={m.id} className="border-border rounded-2xl shadow-card">
                          <CardContent className="p-4 space-y-3">
                            <div className="flex items-center justify-between gap-2">
                              <span className="text-xs text-muted-foreground">
                                <span className="font-semibold text-foreground">{m.requester_name}</span> quiere hacer match
                              </span>
                              <Badge variant={st.variant} className="text-[10px] shrink-0 rounded-full">{st.label}</Badge>
                            </div>

                            <div className="space-y-1.5">
                              <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Tu publicación</p>
                              {renderPubSummary(m.publication)}
                            </div>

                            {m.matched_publication && (
                              <div className="space-y-1.5 border-t border-border pt-3">
                                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Su publicación</p>
                              ...
                              {m.status === "pending" && (
                                <div className="flex gap-2">
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    className="h-8 px-3 text-xs rounded-full border-destructive text-destructive hover:bg-destructive/10"
                ...
                {outgoing.length > 0 && (
                  <div className="space-y-3">
                    <h3 className="text-sm font-bold text-foreground">Matches enviados</h3>
                    {outgoing.map((m) => {
                      const st = statusLabels[m.status] || statusLabels.pending;
                      return (
                        <Card key={m.id} className="border-border rounded-2xl shadow-card">
                          <CardContent className="p-4 space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <Badge variant={st.variant} className="text-[10px] rounded-full">{st.label}</Badge>
                            </div>
                            {renderPubSummary(m.publication)}
                            {m.matched_publication && (
                              <div className="border-t border-border pt-3">
                                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium mb-1.5">Tu publicación vinculada</p>
                                {renderPubSummary(m.matched_publication, true)}
                              </div>
                            )}
                            <span className="text-[10px] text-muted-foreground">{formatDate(m.created_at)}</span>
